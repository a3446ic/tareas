CREATE OR REPLACE PROCEDURE "EXT"."SP_SET_MOVIMIENTOS_ENVIADO_SMM" (IN tipoMov VARCHAR(5), IN IN_FILENAME VARCHAR(120)) LANGUAGE SQLSCRIPT SQL SECURITY DEFINER DEFAULT SCHEMA "EXT" AS
BEGIN
	...
	DECLARE numpoliza BIGINT;
	DECLARE fechaEfecto DATE;
	DECLARE modalidad INT;
	DECLARE pais INT;
	DECLARE numAnualidad SMALLINT;
	DECLARE codigoMediadorHist NVARCHAR(10);
	DECLARE subclaveMediadorHist NVARCHAR(10);
	DECLARE motivo VARCHAR(5000);
	DECLARE valorHist VARCHAR(500);
	DECLARE valorCartera VARCHAR(500);
	DECLARE fechaInicioHist DATE;
	DECLARE fechaEfectoHist DATE;
	DECLARE idSitAnualidad INT;
	...
	FROM EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM
    WHERE ESTADOREG = 'PENDIENTE'
    AND BATCHNAME = IN_FILENAME;

	DECLARE CURSOR mvfid_hist FOR
	SELECT * FROM EXT.EXT_MOVIMIENTO_FIANZAS_CAUCION_HIST_SMM
	WHERE ESTADOREG = 'PENDIENTE'
	AND BATCHNAME = IN_FILENAME;
...
----------------------------- HANDLER EXCEPTION -------------------------
	...
	vProcedure := 'SP_SET_MOVIMIENTOS_ENVIADO_SMM';
...
---Comienza Procedimiento
    ...
    IF tipoMov = 'mvcar' THEN
    	...
    	FOR i AS mvcar_hist DO
			...
			fechaInicio := IFNULL(i.FECHA_INI,'1990-12-31');
			fechaFin := IFNULL(i.FECHA_FIN,'2200-01-01');
			numpoliza:= i.NUM_POLIZA;
			fechaEfecto:= i.FECHA_EFECTO;
			numAnualidad := i.NUM_ANUALIDAD;
			modalidad:= i.IDMODALIDAD;
			pais:= i.IDPAIS;
			codigoMediadorHist := i.IDMEDIADOR;
			subclaveMediadorHist := i.IDSUBCLAVE;
			motivo:= '';
			fechaInicioHist := IFNULL(i.FECHA_INI,'1990-12-31');
			fechaEfectoHist:= i.FECHA_EFECTO;
			idSitAnualidad:= i.ID_SIT_ANUALIDAD;
			-------------------------------------------------------------------------------------------------------
			...
			IF i.IDTIPO_MOV = 2 THEN
				...
				FOR contador IN 0..(registrosRenovacion-1) DO
                    ...
                    SELECT IFNULL(FECHA_INICIO,'1990-12-31'), IFNULL(FECHA_FIN,'2200-01-01') INTO fechaInicio, fechaFin
					...
					IF registroExistente >= 1 THEN --UPDATE
						...
						SELECT 'FLAG1' FROM DUMMY;
							UPDATE EXT.CARTERA_SMM SET
								...
								FECHA_INICIO = IFNULL(fechaInicio,'1990-12-31'),
								--FECHA_FIN = fechaFin,
								FECHA_FIN = IFNULL(fechaFin,'2200-01-01'),
						...
						--SELECT 'UPDATE2' FROM DUMMY;
							UPDATE EXT.CARTERA_SMM SET
								...
								--FECHA_INICIO = i.FECHA_INI,
								--FECHA_INICIO = fechaInicio,
								FECHA_INICIO = IFNULL(fechaInicio,'1900-12-31'),
								--FECHA_FIN = fechaFin,
								FECHA_FIN = IFNULL(fechaFin,'2200-01-01'),
								NIF_TOMADOR = i.IDFISCAL_TOMADOR,
								NOMBRE_TOMADOR = '',  -- NO VIENE EN EL FICHERO
							...
							AND FECHA_INICIO = fechaInicio--IFNULL(i.FECHA_INI,'1990-12-31')
							AND FECHA_EFECTO = IFNULL(i.FECHA_EFECTO,'2200-01-01')
							...
							CALL LIB_GLOBAL_CESCE :w_debug (
						i_Tenant,
						'Update2 línea '|| TO_VARCHAR (numLin) || ' registros actualizados '  || To_VARCHAR(::ROWCOUNT) || ' NUM_POLIZA:' || COALESCE(i.NUM_POLIZA, 0) || ',IDMODALIDAD:' || COALESCE(i.IDMODALIDAD, 0) || ',IDMEDIADOR:' || COALESCE (i.IDMEDIADOR, '0') ||
						',IDSUBCLAVE:' || COALESCE (i.IDSUBCLAVE, '0') || ',FECHA_INI:' || COALESCE (IFNULL(i.FECHA_INI,'1990-12-31'), '0') || 
						',FECHA_FIN' || COALESCE (IFNULL(i.FECHA_EFECTO,'2200-01-01'), '0') || ',TIPO_MOV:' || COALESCE (i.IDTIPO_MOV, 0),
						vProcedure,
						io_contador
						);
						
						/**********************/
						IF To_VARCHAR(::ROWCOUNT) = 0 THEN
							
					--	SELECT codigoMediador,codigoMediadorHist,numpoliza,fechainicio,fechaefecto,numanualidad,subclaveMediador,fechaInicioHist,fechaEfectoHist from dummy;
							--IF codigoMediador != codigoMediadorHist THEN
							IF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza) THEN
						 		SELECT 'FLAG2 '||'DISTINTA POLIZA: ' || numpoliza || ' en la tabla cartera' INTO motivo FROM DUMMY;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist ) THEN
						 		SELECT 'FLAG2 '||'DISTINTO MEDIADOR ',codigoMediadorHist,codigoMediador INTO motivo,valorHist,valorCartera FROM DUMMY;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist) THEN
						 		SELECT 'FLAG2 '||'DISTINTO SUBMEDIADOR',subclaveMediadorHist,subclaveMediador INTO motivo,valorHist,valorCartera FROM DUMMY;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist AND FECHA_INICIO = fechaInicioHist) THEN
						 		SELECT 'FLAG2 '||'DISTINTA FECHA_INICIO',fechaInicioHist,fechaInicio INTO motivo,valorHist,valorCartera FROM DUMMY;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist AND FECHA_INICIO = fechaInicioHist AND FECHA_EFECTO = fechaEfectoHist) THEN
						 		
						 		IF fechainicio != fechaInicioHist THEN
						 			SELECT 'FLAG2 '||'DISTINTA FECHA_INICIO',fechainicioHist,fechainicio INTO motivo,valorHist,valorCartera FROM DUMMY;
						 		ELSEIF fechaEfecto != fechaEfectoHist THEN
						 			SELECT 'FLAG2 '||'DISTINTA FECHA_EFECTO',fechaEfectoHist,fechaEfecto INTO motivo,valorHist,valorCartera FROM DUMMY;
						 		END IF;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist AND FECHA_INICIO = fechaInicioHist AND FECHA_EFECTO = fechaEfectoHist AND NUM_ANUALIDAD = numAnualidad) THEN
						 		SELECT 'FLAG2 '||'DISTINTA ANUALIDAD'  INTO motivo FROM DUMMY;
						 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist AND FECHA_INICIO = fechaInicioHist AND FECHA_EFECTO = fechaEfectoHist AND NUM_ANUALIDAD = numAnualidad AND IDPRODUCT = (SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY) ) THEN
						 		SELECT 'FLAG2'||'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediadorHist || ' - SUBMEDIADOR: ' ||  subclaveMediadorHist || ' - FECHA DE INICIO: ' || fechaInicioHist || ' - FECHA DE EFECTO: ' || fechaEfectoHist || ' - ANUALIDAD: ' || numAnualidad || ' - PRODUCTO: ' || (SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY) || ' en la tabla cartera' INTO motivo FROM DUMMY;
						 	ELSE 
						 		select 'FLAG2'||'¿?' into motivo from dummy;
						 		SELECT IDPRODUCT,(SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY) otro FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediadorHist AND COD_SUBCLAVE = subclaveMediadorHist AND FECHA_INICIO = fechaInicioHist AND FECHA_EFECTO = fechaEfectoHist AND NUM_ANUALIDAD = numAnualidad AND IDPRODUCT = (SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY);
						 		select fechaInicio,fechaInicioHist,numpoliza,fechaEfecto,fechaEfectoHist from dummy;
						 	END IF;
							--	SELECT 'DISTINTOS MEDIADORES' INTO motivo FROM DUMMY;
						--	 select 'insert1',fechainicioHist,MOTIVO from dummy;
								INSERT INTO EXT.MOTIVOS_NO_TRASPASOS_SMM(IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
									FECHA_SIT_ANUALIDAD,FECHA_EMISION,FECHA_EFECTO,FECHA_VENCIMIENTO,IDCLIENTE,NOMBRE_CLIENTE,IND_VIP,IDGRUPO,DESC_GRUPO,IDDELEGACION,
									DESC_DELEGACION,IDCOMERCIAL,NOMBRE_COMERCIAL,IDOFICINA,DESC_OFICINA, IDMEDIADOR, 
									NOMBRE_MEDIADOR,IMPORTE_VENTAS_PREV_EXT,IMPORTE_VENTAS_PREV_INT,PRIMA_PROVISIONAL_INT,PRIMA_PROVISIONAL_EXT,
									PORC_TASA_INT,PORC_TASA_EXT,TIPO_TARIFA_INT,TIPO_TARIFA_EXT,IDMODALIDAD_POL_O,NUM_POL_O,FECHA_TRASPASO_POL_O,
									ID_SIT_POL_O,DESC_SIT_POL_O,IDMODALIDAD_POL_D,NUM_POL_D,FECHA_TRASPASO_POL_D,ID_SIT_POL_D,DESC_SIT_POL_D,FECHA_DATOS,
									IDCNAE,PR_COBRADA_RC_INT,PR_COBRADA_RC_EXT,PR_COBRADA_RP,PR_COBRADA_TC_RC,PR_COBRADA_TC_RP,PR_EMITIDA_RC_INT,PR_EMITIDA_RC_EXT,
									...
									IMPORTE_MAX_FINANCIACION,IMPORTE_FRANQUICIA,INDEMNIZACION_MAX,PORC_TASA_MAX,TARIFA_GASTOS_ANALISIS,
									PRIMA_MIN_MERCADO_INT,PRIMA_MIN_MERCADO_EXT,COD_ACUERDO,IDTIPO_MEDIADIOR,DESC_TIPO_MEDIADIOR,IDSEGMENTO_EMP,DESC_SEGMENTO_EMP,
									IND_FIRMA_DIGITAL,IDAGENTE,FECHA_INI,FECHA_FIN,DESC_SUBCLAVE,IDSUBCLAVE, PORC_INTERMEDIACION,
									IDTIPO_MOV,BATCHNAME,CREATEDATE,ESTADOREG,IDPAIS,MOTIVO,VALORHIST,VALORCARTERA)
								SELECT IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
									FECHA_SIT_ANUALIDAD,FECHA_EMISION,FECHA_EFECTO,FECHA_VENCIMIENTO,IDCLIENTE,NOMBRE_CLIENTE,IND_VIP,IDGRUPO,DESC_GRUPO,IDDELEGACION,
									DESC_DELEGACION,IDCOMERCIAL,NOMBRE_COMERCIAL,IDOFICINA,DESC_OFICINA, LPAD(IDMEDIADOR, 4, '0') as IDMEDIADOR, 
									NOMBRE_MEDIADOR,IMPORTE_VENTAS_PREV_EXT,IMPORTE_VENTAS_PREV_INT,PRIMA_PROVISIONAL_INT,PRIMA_PROVISIONAL_EXT,
									...
									PRIMA_MIN_MERCADO_INT,PRIMA_MIN_MERCADO_EXT,COD_ACUERDO,IDTIPO_MEDIADIOR,DESC_TIPO_MEDIADIOR,IDSEGMENTO_EMP,DESC_SEGMENTO_EMP,
									IND_FIRMA_DIGITAL,IDAGENTE,FECHA_INI,FECHA_FIN,DESC_SUBCLAVE,LPAD(IDSUBCLAVE, 4, '0') as IDSUBCLAVE, PORC_INTERMEDIACION,
									IDTIPO_MOV,BATCHNAME,CURRENT_TIMESTAMP,ESTADOREG,IDPAIS,motivo,valorHist,valorCartera
								FROM EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM 
								WHERE NUM_POLIZA = numpoliza AND BATCHNAME = BATCHNAME AND ESTADOREG='PENDIENTE'
								AND IFNULL(FECHA_INI,'1990-12-31') = fechainicioHist AND FECHA_EFECTO = fechaEfectoHist AND NUMANUALIDAD = numAnualidad AND ID_SIT_ANUALIDAD = idSitAnualidad;
					...
					ELSE --INSERT
						...
						INSERT INTO EXT.CARTERA_SMM VALUES (
							'CREDITO',
							...
							--i.FECHA_INI,
							--fechaInicio,
							IFNULL(fechaInicio,'1990-12-31'),
							--i.FECHA_FIN,
							--fechaFin,
							IFNULL(fechaFin,'2200-01-01'),
					...
					/**********************************************************************************************/
						IF To_VARCHAR(::ROWCOUNT) = 0 THEN
							IF codigoMediador != codigoMediadorHist THEN
								SELECT 'Distintos mediadores: CARTERA ' || codigoMediador || ' --- HIST: ' || codigoMediadorHist INTO motivo FROM DUMMY;
								select 'FLAGXXX ' into motivo from dummy;
								select 'insert2' from dummy;
								INSERT INTO EXT.MOTIVOS_NO_TRASPASOS_SMM(IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
									FECHA_SIT_ANUALIDAD,FECHA_EMISION,FECHA_EFECTO,FECHA_VENCIMIENTO,IDCLIENTE,NOMBRE_CLIENTE,IND_VIP,IDGRUPO,DESC_GRUPO,IDDELEGACION,
									DESC_DELEGACION,IDCOMERCIAL,NOMBRE_COMERCIAL,IDOFICINA,DESC_OFICINA, IDMEDIADOR, 
									NOMBRE_MEDIADOR,IMPORTE_VENTAS_PREV_EXT,IMPORTE_VENTAS_PREV_INT,PRIMA_PROVISIONAL_INT,PRIMA_PROVISIONAL_EXT,
								...
								SELECT IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
									...
									PRIMA_MIN_MERCADO_INT,PRIMA_MIN_MERCADO_EXT,COD_ACUERDO,IDTIPO_MEDIADIOR,DESC_TIPO_MEDIADIOR,IDSEGMENTO_EMP,DESC_SEGMENTO_EMP,
									IND_FIRMA_DIGITAL,IDAGENTE,FECHA_INI,FECHA_FIN,DESC_SUBCLAVE,LPAD(IDSUBCLAVE, 4, '0') as IDSUBCLAVE, PORC_INTERMEDIACION,
									IDTIPO_MOV,BATCHNAME,CREATEDATE,ESTADOREG,IDPAIS,motivo,codigoMediadorHist,codigoMediador 
								FROM EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM 
								WHERE NUM_POLIZA = numpoliza AND BATCHNAME = BATCHNAME AND ESTADOREG='PENDIENTE';
							END IF;
						END IF;
        ...
        SELECT 'UPDATE3' FROM DUMMY;
					...
					io_contador
					 ...
					 IF To_VARCHAR(::ROWCOUNT) = 0 THEN
					 	...
					 	IF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador ) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediador || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediador || ' - SUBMEDIADOR: ' ||  subclaveMediador || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador AND FECHA_INICIO = fechaInicio) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediador || ' - SUBMEDIADOR: ' ||  subclaveMediador || ' - FECHA DE INICIO: ' || fechaInicio || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador AND FECHA_INICIO = fechaInicio AND FECHA_EFECTO = fechaEfecto) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediador || ' - SUBMEDIADOR: ' ||  subclaveMediador || ' - FECHA DE INICIO: ' || fechaInicio || ' - FECHA DE EFECTO: ' || fechaEfecto || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador AND FECHA_INICIO = fechaInicio AND FECHA_EFECTO = fechaEfecto AND NUM_ANUALIDAD = numAnualidad) THEN
					 		SELECT 'DISTINTA ANUALIDAD'  INTO motivo FROM DUMMY;
					 	ELSEIF NOT EXISTS(SELECT 1 FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador AND FECHA_INICIO = fechaInicio AND FECHA_EFECTO = fechaEfecto AND NUM_ANUALIDAD = numAnualidad AND IDPRODUCT = (SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY) ) THEN
					 		SELECT 'NO EXISTE LA POLIZA: ' || numpoliza || ' - CON EL MEDIADOR: ' || codigoMediador || ' - SUBMEDIADOR: ' ||  subclaveMediador || ' - FECHA DE INICIO: ' || fechaInicio || ' - FECHA DE EFECTO: ' || fechaEfecto || ' - ANUALIDAD: ' || numAnualidad || ' - PRODUCTO: ' || (SELECT EXT.LIB_GLOBAL_CESCE :getProductId( (select lpad(modalidad, 3, '0') from dummy), '0', (CASE WHEN (pais > 0 AND pais <= 52) THEN 116 ELSE pais END), numpoliza).productId FROM DUMMY) || ' en la tabla cartera' INTO motivo FROM DUMMY;
					 	END IF;
					 	
					 --	SELECT 'Distintos mediadores: CARTERA ' || codigoMediador || ' --- HIST: ' || codigoMediadorHist INTO motivo FROM DUMMY;
							SELECT 'FLAGX ' INTO motivo FROM DUMMY;
							select 'insert3' from dummy;
							INSERT INTO EXT.MOTIVOS_NO_TRASPASOS_SMM(IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
								FECHA_SIT_ANUALIDAD,FECHA_EMISION,FECHA_EFECTO,FECHA_VENCIMIENTO,IDCLIENTE,NOMBRE_CLIENTE,IND_VIP,IDGRUPO,DESC_GRUPO,IDDELEGACION,
								DESC_DELEGACION,IDCOMERCIAL,NOMBRE_COMERCIAL,IDOFICINA,DESC_OFICINA, IDMEDIADOR, 
								NOMBRE_MEDIADOR,IMPORTE_VENTAS_PREV_EXT,IMPORTE_VENTAS_PREV_INT,PRIMA_PROVISIONAL_INT,PRIMA_PROVISIONAL_EXT,
							...
							SELECT IDMODALIDAD,NUM_POLIZA,IDFASE,DESC_FASE,IDESTADO,DESC_ESTADO,FECHA_SIT,NUM_ANUALIDAD,ID_SIT_ANUALIDAD,DESC_SIT_ANUALIDAD,
								...
								PRIMA_MIN_MERCADO_INT,PRIMA_MIN_MERCADO_EXT,COD_ACUERDO,IDTIPO_MEDIADIOR,DESC_TIPO_MEDIADIOR,IDSEGMENTO_EMP,DESC_SEGMENTO_EMP,
								IND_FIRMA_DIGITAL,IDAGENTE,FECHA_INI,FECHA_FIN,DESC_SUBCLAVE,LPAD(IDSUBCLAVE, 4, '0') as IDSUBCLAVE, PORC_INTERMEDIACION,
								IDTIPO_MOV,BATCHNAME,CREATEDATE,ESTADOREG,IDPAIS,motivo,numAnualidad,(SELECT NUM_ANUALIDAD FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = numpoliza AND COD_MEDIADOR = codigoMediador AND COD_SUBCLAVE = subclaveMediador AND FECHA_INICIO = fechaInicio AND FECHA_EFECTO = fechaEfecto) 
							FROM EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM 
							WHERE NUM_POLIZA = numpoliza AND BATCHNAME = BATCHNAME AND ESTADOREG='PENDIENTE';
		...
		UPDATE EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM SET ESTADOREG = 'ENVIADO'
		WHERE ESTADOREG = 'PENDIENTE';

        COMMIT;
        
        SELECT CASE WHEN (SELECT COUNT(*) FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = H.NUM_POLIZA ) > 0 
		THEN CASE WHEN(SELECT COUNT(*) FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = H.NUM_POLIZA AND COD_MEDIADOR = H.IDMEDIADOR) > 0 
			THEN CASE WHEN H.FECHA_INI IS NOT NULL 
				THEN CASE WHEN (SELECT COUNT(*) FROM EXT.CARTERA_SMM WHERE NUM_POLIZA = H.NUM_POLIZA AND COD_MEDIADOR = H.IDMEDIADOR AND H.FECHA_INI = C.FECHA_INICIO AND H.NUM_ANUALIDAD = C.NUM_ANUALIDAD) > 0 
					THEN 'OK'
					ELSE 'DISTINTA ANUALIDAD'
					END				
				ELSE 'FECHA INICIO ES NULA'
				END
			ELSE 'NO EXISTE MEDIADOR'
			END
		END MOTIVO,
	H.NUM_POLIZA,IDMEDIADOR,C.COD_MEDIADOR,IDSUBCLAVE,FECHA_INI,* FROM EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST_SMM H 
	LEFT JOIN EXT.CARTERA_SMM C ON H.NUM_POLIZA = C.NUM_POLIZA AND H.IDMEDIADOR = C.COD_MEDIADOR AND H.IDSUBCLAVE = C.COD_SUBCLAVE
	AND H.FECHA_INI = C.FECHA_INICIO AND H.NUM_ANUALIDAD = C.NUM_ANUALIDAD
	WHERE 1=1
	AND H.BATCHNAME = 'PRUEBA_MVCAR.TXT'
	--AND H.NUM_POLIZA = VNUM_POLIZA
	;
   ...
   ELSEIF tipoMov = 'mvfid' THEN
		...
		UPDATE EXT.EXT_MOVIMIENTO_FIANZAS_CAUCION_HIST_SMM SET ESTADOREG = 'ENVIADO'
		WHERE ESTADOREG = 'PENDIENTE';
    ...
    'SP_SET_MOVIMIENTOS_ENVIADO_SMM',
    io_contador
);
