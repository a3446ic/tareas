...
-- ----------------------------------------------------------------------------------------------------
		...
		IF (SELECT TABLE_NAME FROM SYS.TABLES where SCHEMA_NAME='EXT' and TABLE_NAME = 'DETALLE_RECIBOS_FACTURAS_TEMP' ) IS NULL THEN
			CREATE COLUMN TABLE "EXT"."DETALLE_RECIBOS_FACTURAS_TEMP"(
				"IDLINEA" BIGINT CS_FIXED NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
				"IDRECIBO" BIGINT CS_FIXED NOT NULL,
				"MATERIAL" VARCHAR(18) NOT NULL,
				"POSITIONNAME" VARCHAR(10) NOT NULL,
				"IMPORTE_DET" DECIMAL(18, 3) CS_FIXED NOT NULL,
				"MONEDA_DET" VARCHAR(5) NOT NULL,
				"ORDERID" VARCHAR(40) DEFAULT '000000' NOT NULL,
				...
				"NUM_RECIBO" INTEGER CS_INT,
				"COD_AVAL" DECIMAL(30) CS_FIXED,
				"PRODUCTID" VARCHAR(20),
				"EVENTTYPEID" VARCHAR(40) NOT NULL,
				"CASEIDTXN" VARCHAR(50),
				"ESTADO_DET" VARCHAR(10) DEFAULT 'PENDIENTE' NOT NULL,
		...
		-- Se insertan los recibos de facturas --
		INSERT INTO EXT.DETALLE_RECIBOS_FACTURAS_TEMP 
		(
			IDRECIBO,
			MATERIAL,
			POSITIONNAME,
		...
		FROM CS_PAYMENT pay
			...
			INNER JOIN CS_APPLDEPOSITPAYMENTTRACE payapd on pay.PAYMENTSEQ = payapd.PAYMENTSEQ
			INNER JOIN CS_APPLIEDDEPOSIT apd on payapd.APPLIEDDEPOSITSEQ = apd.APPLIEDDEPOSITSEQ
			INNER JOIN CS_KPRDEPOSITAPPLDEPOSITTRACE apddep ON apd.APPLIEDDEPOSITSEQ = apddep.APPLIEDDEPOSITSEQ
					AND apd.UNITTYPEFORVALUE = apddep.UNITTYPEFORCONTRIBUTIONVALUE  --V08
					AND apddep.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRDEPOSIT dep on apddep.DEPOSITSEQ = dep.DEPOSITSEQ and dep.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRDEPOSITINCENTIVETRACE depinc on dep.DEPOSITSEQ = depinc.DEPOSITSEQ and depinc.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRINCENTIVE inc on depinc.INCENTIVESEQ = inc.INCENTIVESEQ and inc.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRINCENTIVEPMTRACE incmed on inc.INCENTIVESEQ = incmed.INCENTIVESEQ and incmed.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRMEASUREMENT med on incmed.MEASUREMENTSEQ = med.MEASUREMENTSEQ and med.KPRSEQ=pay.POSTPIPELINERUNSEQ
			LEFT JOIN CS_KPRPMSELFTRACE medmd1 on med.MEASUREMENTSEQ = medmd1.TARGETMEASUREMENTSEQ and medmd1.KPRSEQ=pay.POSTPIPELINERUNSEQ
			LEFT JOIN CS_KPRMEASUREMENT md1 on medmd1.SOURCEMEASUREMENTSEQ = md1.MEASUREMENTSEQ and md1.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRPMCREDITTRACE medcr on ifNULL(md1.MEASUREMENTSEQ, med.MEASUREMENTSEQ) = medcr.MEASUREMENTSEQ and medcr.KPRSEQ=pay.POSTPIPELINERUNSEQ
			INNER JOIN CS_KPRCREDIT cr on medcr.CREDITSEQ = cr.CREDITSEQ and cr.KPRSEQ=pay.POSTPIPELINERUNSEQ
