...
BEGIN
    ...
    BEGIN    	
		...
		SELECT 
	    	...
	    	, ADD_DAYS(ADD_MONTHS(TO_DATE(SUBSTRING(CRH.MESCIERRE,1,4) || '-' || SUBSTRING(CRH.MESCIERRE,5,2) || '-' || '01'),1),-1) FECHA_FACTURA
	    	, CRH.FEC_MOVIMIENTO
	    	, SUBSTRING(CRH.NUM_POLIZA,0,3)
	    	, SUBSTRING(CRH.NUM_POLIZA,4,LENGTH(CRH.NUM_POLIZA)) NUM_POLIZA
	    	, CPTH.TOMADOR TOMADOR -- !!OJO!! Pendiente de obtener valor
	    	, CRH.MESCIERRE
	    	, CRH.IDDIVISA
	    	, (SELECT EXT.LIB_GLOBAL_CESCE:getCurrency(CRH.IDDIVISA,'').currencyISO FROM DUMMY) MONEDA
        ...
        LEFT JOIN EXT.CARGA_POLIZAS_TOMADOR_HIST CPTH
        	ON CPTH.NUM_POLIZA = SUBSTRING(CRH.NUM_POLIZA,4,LENGTH(CRH.NUM_POLIZA))
        WHERE 1=1
	        AND CRH.BATCHNAME LIKE CASE WHEN IN_FILENAME = '' OR IN_FILENAME IS NULL THEN '%%' ELSE IN_FILENAME END
	        AND CRH.ESTADOREG = 'ENVIADA'
	    LIMIT :batch_size offset :offset;
