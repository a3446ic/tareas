CREATE OR REPLACE PROCEDURE EXT.SP_REVISION_CARTERA_OBJ_TEMP_PRD LANGUAGE SQLSCRIPT AS
...
-----------------------------------------------------------------------------------------
        ...
        --COMPRUEBA SI EXISTE MÁS DE UN POSITIONNAME Y ALGUNO ES NULL
            WHEN EXISTS (
                ...
                THEN    CASE WHEN (SELECT COUNT(DISTINCT
                                ...
                                WHERE CR.NUM_POLIZA = C.NUM_POLIZA
                                AND CR.RAMO = 'CREDITO'
                                GROUP BY CR.NUM_POLIZA) > 1 THEN caso33 --3.3 MÁS DE UN POSITIONNAME. ALGUNO NULL
    ...
    -- SELECT * FROM :CTE_RELASUJE WHERE NUM_POLIZA = 1001134;
