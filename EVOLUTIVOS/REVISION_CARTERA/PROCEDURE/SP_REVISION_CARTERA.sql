...
BEGIN
    ...
    IF (SELECT TABLE_NAME FROM SYS.TABLES where SCHEMA_NAME= cEsquema and TABLE_NAME = 'CARTERA_BKP_09042025_FECHAS') IS NULL THEN
	    
        CREATE COLUMN TABLE EXT.CARTERA_BKP_09042025_FECHAS AS (SELECT * FROM EXT.CARTERA)
        UNLOAD PRIORITY 5 AUTO MERGE;

        CALL EXT.LIB_GLOBAL_CESCE :w_debug (
            i_Tenant,
            'CREADA TABLA ' || cEsquema || '.' || 'CARTERA_BKP_09042025_FECHAS',
            'cReport',
            io_contador
        );    
    END IF;
...
-- DELETE FROM EXT.CARTERA WHERE MODIF_USER = 'MANUAL-2';

-- DELETE FROM EXT.CARTERA WHERE MODIF_USER = 'MANUAL-EXPEDIENTE';
...
SELECT * FROM EXT.CARTERA WHERE FECHA_INICIO IS NULL OR FECHA_FIN IS NULL;

SELECT * FROM EXT.CARTERA WHERE MODIF_USER = 'MANUAL-2' AND COD_MEDIADOR = '0004';
END;