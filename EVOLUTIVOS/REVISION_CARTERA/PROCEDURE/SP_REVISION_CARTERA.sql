...
-----------------------------------------------------------------------------------------
    ...
    , CASE
		WHEN EXISTS(
        		SELECT 1 FROM EXT.RELASUJE_20241216 R
				WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 		...
		 		SELECT CASE WHEN FEC_INI IS NULL OR FEC_INI = '0000-00-00' THEN '1990-12-31'ELSE FEC_INI END FROM EXT.RELASUJE_20241216 R
		 		WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 	)
    ...
    , CASE 
		WHEN EXISTS(
				...
				WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 		...
		 		SELECT CASE WHEN FEC_FIN IS NULL OR FEC_FIN = '0000-00-00' THEN '2200-01-01'ELSE FEC_FIN END FROM EXT.RELASUJE_20241216 R
		 		WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 	)
		 	ELSE CT.FECHA_FIN
      END FECHA_FIN_TEMP
    FROM EXT.CARTERA CT INNER JOIN (
        SELECT C.NUM_POLIZA,
        CASE
        --COMPRUEBA QUE EXISTE PÓLIZA EN RELASUJE_20241216
            WHEN NOT EXISTS (
               SELECT 1
               FROM EXT.RELASUJE_20241216 R
               WHERE LPAD(C.NUM_POLIZA,8,0) = R.NUM_POLIZA
           ) THEN caso10 --1.0 NO EXISTE PÓLIZA

        --COMPRUEBA QUE TIENE LA MISMA MODALIDAD EN RELASUJE_20241216
            WHEN NOT EXISTS (
                SELECT 1
                FROM EXT.CARTERA CR
                INNER JOIN EXT.RELASUJE_20241216 R ON LPAD(CR.NUM_POLIZA,8,0) = R.NUM_POLIZA
        ...
        --COMPRUEBA SI EXISTE MÁS DE UN POSITIONNAME Y ALGUNO ES NULL
            WHEN EXISTS (
                SELECT 1 
                FROM EXT.CARTERA CR INNER JOIN EXT.RELASUJE_20241216 R ON LPAD(CR.NUM_POLIZA,8,0) = R.NUM_POLIZA
    ...
    LEFT JOIN EXT.RELASUJE_20241216 R ON LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA
    ...
    , CASE
		WHEN EXISTS(
				...
				WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 		...
		 		SELECT CASE WHEN MAX(FEC_FIN) IS NULL OR MAX(FEC_FIN) = '0000-00-00' THEN '2200-01-01'ELSE MAX(FEC_FIN) END FROM EXT.RELASUJE_20241216 R
		 		WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 	)
		 	ELSE CT.FECHA_FIN
      END FECHA_FIN_TEMP
    ...
    , CASE
		WHEN EXISTS(
				...
				WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 		...
		 		SELECT CASE WHEN MAX(FEC_INI) IS NULL OR MAX(FEC_INI) = '0000-00-00' THEN '1990-12-31'ELSE MAX(FEC_INI) END FROM EXT.RELASUJE_20241216 R
		 		WHERE LPAD(CT.NUM_POLIZA,8,0) = R.NUM_POLIZA AND R.POSITIONNAME IS NOT NULL AND CT.COD_MEDIADOR||'-'||CT.COD_SUBCLAVE = R.POSITIONNAME
		 	)
		 	ELSE CT.FECHA_INICIO
      END FECHA_INICIO_TEMP
	...
	SELECT RAMO
		...
		, CASE WHEN 1 = (SELECT DISTINCT 1 FROM EXT.CARTERA C
		    				INNER JOIN EXT.RELASUJE_20241216 R ON 
