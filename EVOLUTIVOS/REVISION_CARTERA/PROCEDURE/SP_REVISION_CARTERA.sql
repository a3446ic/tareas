...
-----------------------------------------------------------------------------------------
    ...
    , CASE
        WHEN C2.MODIF_CASE IN (caso01,caso10) THEN NULL
        WHEN C2.MODIF_CASE IN (caso11,caso12) AND (R.FEC_INI IS NULL OR R.FEC_INI = '0000-00-00') THEN '1990-12-31' ELSE R.FEC_INI END FECHA_INICIO_TEMP
    , CASE 
        WHEN C2.MODIF_CASE IN (caso01,caso10) THEN NULL
        WHEN C2.MODIF_CASE IN (caso11,caso12) AND (R.FEC_FIN IS NULL OR R.FEC_FIN = '0000-00-00') THEN '2200-01-01' ELSE R.FEC_FIN END FECHA_FIN_TEMP 
    ...
    INSERT INTO EXT.CARTERA_OBJ_TEMP
    SELECT CT.*,C2.MODIF_CASE,CT.FECHA_INICIO,
        CASE WHEN CT.ACTIVO = 2 THEN COALESCE(CT.FECHA_FIN, DATE '2200-01-01') ELSE CT.FECHA_FIN END FROM EXT.CARTERA CT INNER JOIN (
        SELECT C.NUM_POLIZA,
        CASE
            WHEN NOT EXISTS (
               SELECT 1 
