CREATE OR REPLACE PROCEDURE EXT.SP_REVISION_CARTERA_OBJ_TEMP LANGUAGE SQLSCRIPT AS
...
/*
	...
	| 
	| Version: 1	
	| Version: 2. Añadidos los campos FECHA_INICIO_OPESP y FECHA_FIN_OPESP. 
    | ¡¡¡¡ ATENCIÓN !!!! FALTA AÑADIR LOS CAMPOS DE FECHAS OPESP EN PRODUCCIÓN
	| 
	|
	----------------------------------------------------------------------------------------------- 
*/
...
BEGIN
    ...
    -- Constantes
    DECLARE cReport CONSTANT VARCHAR(50) := 'SP_REVISION_CARTERA_OBJ_TEMP';
...
-----------------------------------------------------------------------------------------
    ...
    , CASE 
		WHEN EXISTS(
		 	...
		 	ELSE CT.FECHA_FIN
      END FECHA_FIN_TEMP
    FROM EXT.CARTERA_BKP_SMM CT INNER JOIN (
        SELECT C.NUM_POLIZA,
        CASE
        ...
        --COMPRUEBA SI EXISTE MÁS DE UN POSITIONNAME Y ALGUNO ES NULL
            WHEN EXISTS (
                SELECT 1 
                FROM EXT.CARTERA_BKP_SMM CR INNER JOIN EXT.RELASUJE R ON LPAD(CR.NUM_POLIZA,8,0) = R.NUM_POLIZA
        ...
        --COMPRUEBA SI TIENEN EL MISMO MEDIADOR
            WHEN EXISTS (SELECT R.POSITIONNAME FROM EXT.CARTERA_BKP_SMM CR
                        INNER JOIN EXT.RELASUJE R ON LPAD(CR.NUM_POLIZA,8,0) = R.NUM_POLIZA
                        AND LPAD(CR.IDMODALIDAD,3,0) = R.MOD
                        WHERE CR.NUM_POLIZA = C.NUM_POLIZA
    ...
    AND NOT EXISTS (
      SELECT 1 
      FROM EXT.CARTERA_BKP_SMM C_SUB
...
--EXPEDIENTES
	...
	SELECT RAMO
		...
		, IDMODALIDAD
    	, 0 NUM_ANUALIDAD
		, CASE WHEN 1 = (SELECT DISTINCT 1 FROM EXT.CARTERA_BKP_SMM C
	    				...
	    				THEN (SELECT DISTINCT CASE WHEN MIN(FEC_INI) IS NULL OR MIN(FEC_INI) = '0000-00-00' THEN '1990-12-31'ELSE MIN(FEC_INI) END FROM EXT.CARTERA_BKP_SMM C
		    				INNER JOIN EXT.RELASUJE R ON 
		    				LPAD(C.NUM_POLIZA,8,0) = R.NUM_POLIZA 
		    				AND LPAD(C.IDMODALIDAD,3,0) = R.MOD
		    				AND C.COD_MEDIADOR || '-' || C.COD_SUBCLAVE = R.POSITIONNAME
	        ...
	        , CASE WHEN 1 = (SELECT DISTINCT 1 FROM EXT.CARTERA_BKP_SMM C
	    				...
	    				THEN (SELECT DISTINCT CASE WHEN MAX(FEC_FIN) IS NULL OR MAX(FEC_FIN) = '0000-00-00' THEN '2200-01-01'ELSE MAX(FEC_FIN) END FROM EXT.CARTERA_BKP_SMM C
		    				INNER JOIN EXT.RELASUJE R ON 
		    				LPAD(C.NUM_POLIZA,8,0) = R.NUM_POLIZA 
		    				AND LPAD(C.IDMODALIDAD,3,0) = R.MOD
		    				AND C.COD_MEDIADOR || '-' || C.COD_SUBCLAVE = R.POSITIONNAME
	...
	WHERE RAMO = 'CAUCION' 
	AND 1 = (CASE WHEN EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_SMM WHERE RAMO = 'CAUCION' AND NUM_POLIZA = CT.NUM_POLIZA AND IDMODALIDAD = CT.IDMODALIDAD --AND FECHA_INICIO = CT.FECHA_INICIO AND FECHA_FIN = CT.FECHA_FIN
...
DO BEGIN
	...
	SELECT * FROM EXT.CARTERA_OBJ_TEMP;
END;