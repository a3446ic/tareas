CREATE OR REPLACE FUNCTION EXT.GET_POLIZAS_CREDITO_ERROR_TEST(i_FILENAME VARCHAR(250)) 
/*
	----------------------------------------------------------------------------------------------- 
...
*/
	...
	BEGIN
		RETURN SELECT DISTINCT
				CASE 
					WHEN NOT EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_TEST WHERE RAMO = 'CREDITO' AND NUM_POLIZA = M.NUM_POLIZA) 
						THEN 'PÓLIZA NO REGISTRADA EN CARTERA'
					WHEN NOT EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_TEST WHERE RAMO = 'CREDITO' AND NUM_POLIZA = M.NUM_POLIZA AND NUM_ANUALIDAD = M.NUM_ANUALIDAD) 
					...
					WHEN NOT EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_TEST WHERE RAMO = 'CREDITO' AND NUM_POLIZA = M.NUM_POLIZA AND NUM_ANUALIDAD = M.NUM_ANUALIDAD
					AND LPAD(M.IDMEDIADOR,4,0) ||'-'|| LPAD(M.IDSUBCLAVE,4,0) = LPAD(COD_MEDIADOR,4,0) ||'-'|| LPAD(COD_SUBCLAVE,4,0) AND M.FECHA_EFECTO = FECHA_EFECTO) 
			...
			AND NOT EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_TEST WHERE RAMO = 'CREDITO' AND NUM_POLIZA = M.NUM_POLIZA AND NUM_ANUALIDAD = M.NUM_ANUALIDAD
						AND LPAD(M.IDMEDIADOR,4,0) ||'-'|| LPAD(M.IDSUBCLAVE,4,0) = LPAD(COD_MEDIADOR,4,0) ||'-'|| LPAD(COD_SUBCLAVE,4,0) AND M.FECHA_EFECTO = FECHA_EFECTO AND M.FECHA_VENCIMIENTO = FECHA_VENCIMIENTO)
			-- v02: filtramos período de intermediación fuera de la fecha de vigencia 
			AND 1 = ( CASE WHEN EXISTS(SELECT 1 FROM EXT.CARTERA_BKP_TEST WHERE RAMO = 'CREDITO' AND NUM_POLIZA = M.NUM_POLIZA AND NUM_ANUALIDAD = M.NUM_ANUALIDAD 
