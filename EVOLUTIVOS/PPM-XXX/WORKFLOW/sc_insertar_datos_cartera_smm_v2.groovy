...
def receptores = mapa['receptores'] ?: []  // Si es null, usa lista vacía
// Mapa de polizas
def polizas = mapa['polizas'] ?: []  // Si es null, usa lista vacía
def numPolizas = polizas.collect { it.num_poliza }
def jsonPolizas = JsonOutput.toJson(numPolizas);
// Mapa de avales
...
switch(tipoMovimiento) {
            ...
            case 'credito':
                switch(derechosObligaciones) {
                    case 'traspaso_con_derechos_y_obligaciones':
                        logger.info('MEDIADOR-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CREDITO');           
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_credito('$json')""";
                        break;
                    case 'traspaso_sin_derechos_y_obligaciones':
                        logger.info('MEDIADOR-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito('$json')""";
                        break;
                    case 'sin_derechos_y_obligaciones_al_inicio_de_la_anuali':
                        logger.info('MEDIADOR-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA ANUALIDAD ACTUAL - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_anualidad_actual_credito('$json')""";
                        break;
                }
                break;
            case 'caucion':
                switch(derechosObligaciones) {
                    case 'traspaso_con_derechos_y_obligaciones':
                        logger.info('MEDIADOR-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_caucion('$json')""";
                        break;
                    case 'traspaso_sin_derechos_y_obligaciones':
                        logger.info('MEDIADOR-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_caucion('$json')""";
                        break;
                }
                break;
            case 'ambos':
                switch(derechosObligaciones) {
                    case 'traspaso_con_derechos_y_obligaciones':
                        ...
                        logger.info('MEDIADOR-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CREDITO');           
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_credito('$json')""";
                        logger.info('queryTraspaso ' + queryTraspaso);
                        db.execute(queryTraspaso);
                        //CAUCION
                        logger.info('MEDIADOR-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_caucion('$json')""";
                        break;
        ...
        switch(ramo){
            ...
            case 'ambos':
                switch(derechosObligaciones) {
                    case 'traspaso_con_derechos_y_obligaciones':
                        //CREDITO
                        logger.info('SIN-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_credito('$json')""";
                        logger.info('queryTraspaso ' + queryTraspaso);
                        db.execute(queryTraspaso);
                        //CAUCION
                        logger.info('SIN-MEDIADOR - CON DERECHOS Y OBLIGACIONES - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_con_derechos_caucion('$json')""";
                        break;
                    case 'traspaso_sin_derechos_y_obligaciones':
                        //CREDITO
                        logger.info('SIN-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito('$json')""";
                        logger.info('queryTraspaso ' + queryTraspaso);
                        db.execute(queryTraspaso);
                        //CAUCION
                        logger.info('SIN-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_caucion('$json')""";
                        break;
                    case 'sin_derechos_y_obligaciones_al_inicio_de_la_anuali':
                        //CREDITO
                        logger.info('SIN-MEDIADOR - SIN DERECHOS Y OBLIGACIONES A LA ANUALIDAD ACTUAL - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_anualidad_actual_credito('$json')""";
                        break;
                }
                break;
    ...
    case 'TRASPASO_%_MEDIADOR':
        //TRASPASO % MEDIADOR
        break;
    
    case 'ERROR_CAPTURA_MEDIADOR':
        //ERROR_CAPTURA_MEDIADOR
        break;
    ...
    case 'MEDIADOR-CANAL-DIRECTO':
        ...
        switch(ramo){
            case 'credito':
                switch(derechosObligaciones) {
                    //SOLO SIN DERECHOS Y OBLIGACIONES
                    case 'traspaso_sin_derechos_y_obligaciones':
                        logger.info('MEDIADOR-CANAL-DIRECTO - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito('$json')""";
                        break;
                    case 'sin_derechos_y_obligaciones_al_inicio_de_la_anuali':
                        logger.info('MEDIADOR-CANAL-DIRECTO - SIN DERECHOS Y OBLIGACIONES A LA ANUALIDAD ACTUAL - CREDITO');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_anualidad_actual_credito('$json')""";
                        break;
                }
                break;
            case 'caucion':
                switch(derechosObligaciones) {
                    //SOLO SIN DERECHOS Y OBLIGACIONES
                    case 'traspaso_sin_derechos_y_obligaciones':
                        logger.info('MEDIADOR-CANAL-DIRECTO - SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN - CAUCION');
                        queryTraspaso = """CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_caucion('$json')""";
                        break;
                }
                break;
...
logger.info('sc_insertar_datos_cartera_smm: Termina el script.');
...
// if(tipoMovimiento == 'finalizar_relacion_intermediacion'){
...
//   }else if(tipoFechaTraspaso == 'traspaso_sin_derechos_y_obligaciones'){ // Renovación
