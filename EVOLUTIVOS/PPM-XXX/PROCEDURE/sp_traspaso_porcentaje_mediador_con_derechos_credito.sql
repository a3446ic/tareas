CREATE OR REPLACE PROCEDURE EXT.sp_traspaso_porcentaje_mediador_con_derechos_credito (IN caseId BIGINT, IN nombreCasoOrigen VARCHAR(100))
LANGUAGE SQLSCRIPT 
AS
/*
	----------------------------------------------------------------------------------------------- 
	...
	|---------------------------------------------------------------------------------------------- 
	| Procedure Purpose: TRASPASO % A OTRO MEDIADOR CON DERECHOS Y OBLIGACIONES
	| 
	| Version: 1
...
BEGIN
    ...
    -- CONSTANTES
    DECLARE cReport CONSTANT VARCHAR(250) := ::CURRENT_OBJECT_NAME;
