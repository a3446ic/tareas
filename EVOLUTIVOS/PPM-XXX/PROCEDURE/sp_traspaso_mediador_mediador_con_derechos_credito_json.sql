CREATE OR REPLACE PROCEDURE EXT.sp_traspaso_mediador_mediador_con_derechos_credito_json (IN p_json NVARCHAR(5000))
LANGUAGE SQLSCRIPT 
AS
/*
	----------------------------------------------------------------------------------------------- 
...
BEGIN
 ...
 ------------------------------- HANDLER EXCEPTION -------------------------
   ...
   -- SELECT v_caseId,v_fechaTraspaso,v_codMediadorCedente,v_subClaveMediadorCedente,v_tipoTraspaso FROM DUMMY;
	...
	DO
	    -- select CR.CODIGOMEDIADORRECEPTOR,CR.SUBCLAVEMEDIADORRECEPTOR,CR.PORCENTAJE_INTERMEDIACION_RECEPTOR,
	    -- CR.FECHATRASPASO,CR.CODIGOMEDIADORCEDENTE,CR.subClaveMediadorCedente from dummy;
        ...
        AND COD_SUBCLAVE = CR.subClaveMediadorCedente
        AND RAMO = 'CREDITO'
        AND ACTIVO = 1
...
SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_ANUALIDAD,COD_MEDIADOR,COD_SUBCLAVE,FECHA_INICIO,FECHA_FIN FROM EXT.CARTERA WHERE COD_MEDIADOR = '0004' AND RAMO = 'CREDITO' ORDER BY ACTIVO,COD_MEDIADOR,NUM_POLIZA,NUM_ANUALIDAD;
...
SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_ANUALIDAD,COD_MEDIADOR,COD_SUBCLAVE,P_INTERMEDIACION,FECHA_INICIO,FECHA_FIN,MODIF_SOURCE,MODIF_DATE FROM EXT.CARTERA WHERE (COD_MEDIADOR = '0004' OR MODIF_USER = 'SMM') AND RAMO = 'CREDITO' ORDER BY ACTIVO,COD_MEDIADOR,NUM_ANUALIDAD,NUM_POLIZA;
