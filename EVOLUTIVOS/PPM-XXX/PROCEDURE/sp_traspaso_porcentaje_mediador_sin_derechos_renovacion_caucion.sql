CREATE OR REPLACE PROCEDURE EXT.sp_traspaso_porcentaje_mediador_sin_derechos_renovacion_caucion (IN caseId BIGINT, IN nombreCasoOrigen VARCHAR(100))
LANGUAGE SQLSCRIPT 
AS
/*
	----------------------------------------------------------------------------------------------- 
	...
	|---------------------------------------------------------------------------------------------- 
	| Procedure Purpose: TRASPASO % DE UN MEDIADOR A OTRO MEDIADOR SIN DERECHOS Y OBLIGACIONES A LA RENOVACIÓN CAUCIÓN
	| 
	| Version: 1	
	|
	| 
...
-- DECLARACION DE VARIABLES
    ...
    -- CONSTANTES
    DECLARE cReport CONSTANT VARCHAR(150) := 'sp_traspaso_porcentaje_mediador_sin_derechos_renovacion_caucion';
...
DECLARE vCASEID BIGINT = 1981;

TRUNCATE TABLE EXT.CARTERA;
INSERT INTO EXT.CARTERA SELECT *,NULL,NULL FROM EXT.CARTERA_BKP_SMM;
CALL EXT.SP_REVISION_CARTERA;

DELETE FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_porcentaje_mediador_sin_derechos_renovacion_caucion%';
...
CALL EXT.sp_traspaso_porcentaje_mediador_sin_derechos_renovacion_caucion(vCASEID,'Traspaso Pólizas y Avales-GM-426');
...
FROM EXT.CARTERA 
WHERE COD_MEDIADOR IN('0004','3071')  AND RAMO = vRAMO ORDER BY ACTIVO,COD_MEDIADOR,NUM_AVAL_HOST,NUM_POLIZA;


SELECT * FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_porcentaje_mediador_sin_derechos_renovacion_caucion%';
