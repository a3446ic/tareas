...
BEGIN
	...
	DECLARE cVersion CONSTANT VARCHAR(2) := '20';
    ...
    IF tipoMov = 'mvcar' THEN
    	...
    	FOR i AS mvcar_hist DO
			...
			IF i.IDTIPO_MOV = 2 THEN
				...
				FROM EXT.CARTERA
				WHERE NUM_POLIZA = i.NUM_POLIZA
				AND LPAD(IDPRODUCT,3,0) = (SELECT EXT.LIB_GLOBAL_CESCE:getProductId((select lpad(i.IDMODALIDAD, 3, '0') from dummy), '0', (CASE WHEN (i.IDPAIS > 0 AND i.IDPAIS <= 52) THEN 116 ELSE i.IDPAIS END), lpad(i.NUM_POLIZA , 8, '0')).productId FROM DUMMY)
				AND ACTIVO = 2
   ...
   ELSEIF tipoMov = 'mvfid' THEN
		...
		FOR i AS mvfid_hist DO
			...
			IF i.IDTIPO_MOV = 2 THEN
				...
				AND ACTIVO = 2
				AND LPAD(IDPRODUCT,3,0) = (SELECT EXT.LIB_GLOBAL_CESCE:getProductId( (select lpad(i.IDMODALIDAD, 3, '0') from dummy), i.IDSUBMODALIDAD, (CASE WHEN (i.IDPAIS > 0 AND i.IDPAIS <= 52) THEN '116' ELSE i.IDPAIS END), lpad(i.NUM_POLIZA , 8, '0')).productId FROM DUMMY);
				...
				FOR contador IN 0..(registrosRenovacion-1) DO
					...
					IF registroExistente >= 1 THEN
						...
						UPDATE EXT.CARTERA SET
							...
							MODIF_USER = 'CDL',
							MODIF_SOURCE = IN_FILENAME
						WHERE LPAD(IDPRODUCT,3,0) = (SELECT EXT.LIB_GLOBAL_CESCE:getProductId((select lpad(i.IDMODALIDAD, 3, '0') from dummy), i.IDSUBMODALIDAD, (CASE WHEN (i.IDPAIS > 0 AND i.IDPAIS <= 52) THEN '116' ELSE i.IDPAIS END), lpad(i.NUM_POLIZA , 8, '0')).productId FROM DUMMY)
