CREATE OR REPLACE PROCEDURE EXT.sp_traspaso_mediador_mediador_con_derechos_credito_plan_comisionamiento_test (IN caseId BIGINT, IN nombreCasoOrigen VARCHAR(100))
LANGUAGE SQLSCRIPT 
AS
/*
	----------------------------------------------------------------------------------------------- 
...
BEGIN
    ...
    FOR CT AS CURSOR_TRASPASOS DO
        ...
        CRT.FECHA_EFECTO,--FECHA_INICIO
        '2200-01-01',--FECHA_FIN
        CASE WHEN
        	CRT.NUM_ANUALIDAD IN (0,1)  AND CRT.NUM_ANUALIDAD = C.NUM_ANUALIDAD THEN PP.P_EMISION_FIN
        		ELSE NULL
        END, --P_ESPECIAL_EMISION
        CASE 
        	--ANUALIDAD VIGENTE TIENE P_ESPECIAL_RENOVACION
        	WHEN CRT.NUM_ANUALIDAD NOT IN (0,1)  AND CRT.NUM_ANUALIDAD = C.NUM_ANUALIDAD AND (C.P_ESPECIAL_RENOVACION IS NOT NULL OR C.P_ESPECIAL_RENOVACION = 0) THEN C.P_ESPECIAL_RENOVACION
        	--ANUALIDAD VIGENTE TIENE P_ESPECIAL_EMISION Y NO TIENE P_ESPECIAL_RENOVACION
        	WHEN CRT.NUM_ANUALIDAD NOT IN (0,1)  AND CRT.NUM_ANUALIDAD = C.NUM_ANUALIDAD AND C.P_ESPECIAL_EMISION IS NULL AND (C.P_ESPECIAL_RENOVACION IS NULL OR C.P_ESPECIAL_RENOVACION = 0) THEN PP.P_RENOV_FIN 
		...
		nombreCasoOrigen,
        CASE WHEN
        	CRT.NUM_ANUALIDAD = C.NUM_ANUALIDAD AND ((PP.P_EMISION_FIN IS NOT NULL AND PP.P_EMISION_FIN <> 0)  OR (PP.P_RENOV_FIN IS NOT NULL AND PP.P_RENOV_FIN <> 0)) THEN
        		C.FECHA_EFECTO
...
DECLARE vCASEID BIGINT = 2111;

TRUNCATE TABLE EXT.CARTERA;
INSERT INTO EXT.CARTERA SELECT * FROM EXT.CARTERA_BKP_20250623 ;
DELETE FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_mediador_mediador_con_derechos_credito%' AND cast(DATETIME AS DATE) = CURRENT_DATE;

SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_AVAL_HOST,COD_MEDIADOR,COD_SUBCLAVE,FECHA_INICIO,FECHA_FIN
, P_ESPECIAL_EMISION, P_ESPECIAL_RENOVACION FROM EXT.CARTERA WHERE COD_MEDIADOR = '2004' AND COD_SUBCLAVE = '0000' AND RAMO = vRAMO ORDER BY ACTIVO,COD_MEDIADOR,NUM_POLIZA,NUM_AVAL_HOST;

	
CALL EXT.sp_traspaso_mediador_mediador_con_derechos_credito_plan_comisionamiento_test(2111,'Traspaso Pólizas y Avales-GM-490');

SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_ANUALIDAD,COD_MEDIADOR,COD_SUBCLAVE,P_INTERMEDIACION,FECHA_EFECTO,FECHA_EMISION,FECHA_VENCIMIENTO,FECHA_INICIO,FECHA_FIN,MODIF_SOURCE,MODIF_DATE,MODIF_USER
, P_ESPECIAL_EMISION, P_ESPECIAL_RENOVACION,FECHA_INICIO_OPESP, FECHA_FIN_OPESP
FROM EXT.CARTERA 
WHERE 1=1
AND NUM_POLIZA = 22751
AND MODIF_USER LIKE '%2111%'
ORDER BY COD_MEDIADOR,NUM_POLIZA
;
...
SELECT * FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_mediador_mediador_con_derechos_credito%' AND cast(DATETIME AS DATE) = CURRENT_DATE;
