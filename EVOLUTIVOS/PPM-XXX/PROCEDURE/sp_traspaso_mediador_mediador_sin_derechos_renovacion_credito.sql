...
DECLARE vCASEID BIGINT = 2123;

TRUNCATE TABLE EXT.CARTERA;
INSERT INTO EXT.CARTERA SELECT * FROM EXT.CARTERA_BKP_SMM ;
DELETE FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito%' AND CAST(DATETIME AS DATE) = CURRENT_DATE;

SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_AVAL_HOST,COD_MEDIADOR,COD_SUBCLAVE,P_ESPECIAL_EMISION,P_ESPECIAL_RENOVACION,P_INTERMEDIACION,FECHA_INICIO,FECHA_FIN 
FROM EXT.CARTERA 
WHERE NUM_POLIZA = 9045417 AND RAMO = vRAMO 
ORDER BY ACTIVO,COD_MEDIADOR,NUM_POLIZA,NUM_AVAL_HOST;

	
CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito(vCASEID,'XXX');

SELECT ACTIVO,
--IDPAIS,
IDPRODUCT,NUM_POLIZA
--,NUM_AVAL_HOST
,COD_MEDIADOR,COD_SUBCLAVE,P_ESPECIAL_EMISION,P_ESPECIAL_RENOVACION,P_INTERMEDIACION,FECHA_INICIO,FECHA_FIN,MODIF_SOURCE,MODIF_DATE,MODIF_USER 
FROM EXT.CARTERA 
WHERE NUM_POLIZA = 9045417 AND RAMO = vRAMO 
AND RAMO = vRAMO ORDER BY ACTIVO,COD_MEDIADOR,NUM_AVAL_HOST,NUM_POLIZA;


SELECT * FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%sp_traspaso_mediador_mediador_sin_derechos_renovacion_credito%' AND CAST(DATETIME AS DATE) = CURRENT_DATE;
