...
/*
	...
	| 
	| Version: 1
	| V 1.1 20251110: Se añade condicicón ACTIVO > 0, para permitir traspasos con registros activo = 2
...
BEGIN
    ...
    DECLARE cVersion  CONSTANT VARCHAR(5) :='2';
	...
	SET FECHA_FIN = (SELECT FECHA_VENCIMIENTO
		...
		, ACTIVO = CASE WHEN C.ACTIVO = 2 THEN 0 ELSE C.ACTIVO END -- 20251110 se pone activo = 0 si se vuelve a traspasar un activo = 2, caso contrario se queda igual
	FROM EXT.CARTERA C
	WHERE C.COD_MEDIADOR = v_codMediadorCedente
	AND C.COD_SUBCLAVE = v_subClaveMediadorCedente
	AND RAMO = cRamo
	--AND ACTIVO = 1 20251110
	AND ACTIVO > 0
