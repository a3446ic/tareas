...
BEGIN
	...
	DO
        ...
        INNER JOIN (SELECT *
					...
					FROM EXT.CARTERA CR WHERE CR.COD_MEDIADOR = CT.COD_MEDIADOR_CEDENTE AND CR.COD_SUBCLAVE = CT.SUBCLAVE_CEDENTE AND CR.NUM_POLIZA = CT.NUM_POLIZA AND CR.RAMO = cRamo AND CR.ACTIVO = 1
					AND CR.NUM_POLIZA = CT.NUM_POLIZA
        	        ) CRT ON C.NUM_POLIZA = CRT.NUM_POLIZA AND C.COD_MEDIADOR = CRT.COD_MEDIADOR AND C.COD_SUBCLAVE = CRT.COD_SUBCLAVE AND C.NUM_ANUALIDAD = CRT.NUM_ANUALIDAD AND CRT.RN = 1
	...
	UPDATE C
	SET FECHA_FIN = (SELECT ADD_DAYS(CT.FECHA_EFECTO,-1) 
					FROM (SELECT FECHA_EFECTO,ROW_NUMBER() OVER (PARTITION BY NUM_POLIZA, COD_MEDIADOR, COD_SUBCLAVE ORDER BY NUM_ANUALIDAD DESC) AS RN
							...
							WHERE C.COD_MEDIADOR = CART.COD_MEDIADOR AND C.COD_SUBCLAVE = CART.COD_SUBCLAVE AND C.NUM_POLIZA = CART.NUM_POLIZA 
								AND CART.RAMO = cRamo AND CART.ACTIVO = 1) CT
					WHERE RN = 1)
	...
	INNER JOIN (SELECT *
				...
				FROM EXT.CARTERA CART 
				WHERE COD_MEDIADOR = CART.COD_MEDIADOR AND COD_SUBCLAVE = CART.COD_SUBCLAVE AND NUM_POLIZA = CART.NUM_POLIZA AND RAMO = cRamo AND ACTIVO = 1
...
DELETE FROM EXT.CARTERA WHERE NUM_POLIZA = 1001297 AND RAMO = 'CREDITO';
INSERT INTO EXT.CARTERA SELECT * FROM EXT.CARTERA_BKP_20250429 WHERE NUM_POLIZA = 1001297 AND RAMO = 'CREDITO';
CALL EXT.SP_REVISION_CARTERA();
SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_ANUALIDAD,NUM_AVAL_HOST,COD_MEDIADOR,COD_SUBCLAVE,FECHA_INICIO,FECHA_FIN FROM EXT.CARTERA WHERE COD_MEDIADOR = '0079'
AND COD_SUBCLAVE = '0000' AND RAMO = vRAMO AND NUM_POLIZA = 1001297 ORDER BY ACTIVO,COD_MEDIADOR,NUM_POLIZA,NUM_AVAL_HOST;

	
CALL EXT.sp_traspaso_mediador_mediador_sin_derechos_anualidad_actual_credito(vCASEID,'CASEID '||vCASEID);

SELECT ACTIVO,IDPAIS,IDPRODUCT,NUM_POLIZA,NUM_ANUALIDAD,COD_MEDIADOR,COD_SUBCLAVE,FECHA_EFECTO,P_INTERMEDIACION,FECHA_INICIO,FECHA_FIN,MODIF_SOURCE,MODIF_DATE,MODIF_USER
,FECHA_EFECTO_TRASPASO
FROM EXT.CARTERA 
--WHERE (COD_MEDIADOR IN('0079') OR MODIF_USER = 'SMM') AND RAMO = vRAMO 
WHERE NUM_POLIZA = 1001297 AND RAMO = 'CREDITO'
AND (COD_MEDIADOR = '0079' AND COD_SUBCLAVE = '0000') OR COD_MEDIADOR = '3071'
