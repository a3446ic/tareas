...
v_posFin := INSTR(IN_FILENAME, '_', 1, 3);
 ...
 --SELECT TO_DATE(SUBSTR(IN_FILENAME,v_posIni,8),'YYYYMMDD') INTO  v_fechaPago FROM DUMMY;
SELECT 
...
INSERT INTO EXT.PAGO_FACTURAS(IDENTIFIER,IDFACTURA,ESTADO,FECHA_VALOR,CHECK_NUMBER,BATCHNAME)
SELECT IDENTIFIER,IDFACTURA,ESTADO,FECHA_VALOR,CHECK_NUMBER,IN_FILENAME
FROM (
SELECT 
  --,SUBSTR_BEFORE(C.LINEA, ';')  AS CAMPO1
  SUBSTR_BEFORE(RF.LINEA, ';') AS IDENTIFIER
  ,SUBSTR_BEFORE(SUBSTR_AFTER(RF.LINEA, ';'),';') AS IDFACTURA
  ,SUBSTR_BEFORE(SUBSTR_AFTER(SUBSTR_AFTER(RF.LINEA, ';'),';'),';') AS ESTADO
  ,TO_VARCHAR(TO_DATE(SUBSTR_BEFORE(SUBSTR_AFTER(SUBSTR_AFTER(SUBSTR_AFTER(RF.LINEA, ';'),';'),';'),';'),'DD/MM/YYYY'),'YYYY-MM-DD')  AS FECHA_VALOR
  ,SUBSTR_AFTER(SUBSTR_AFTER(SUBSTR_AFTER(SUBSTR_AFTER(RF.LINEA, ';'),';'),';'),';') AS CHECK_NUMBER
FROM EXT.PAGO_FACTURAS_LOAD RF
...
IF EXISTS(SELECT 1 FROM EXT.PAGO_FACTURAS WHERE ESTADO = 'REJECTED' AND BATCHNAME = IN_FILENAME) THEN
	CALL LIB_GLOBAL_CESCE :w_debug (
    i_Tenant,
    'EL FICHERO CONTIENE PAGOS SIN EFECTUAR',
    v_proc_name,
    io_contador
);
...
UPDATE F SET FECHA_PAGO = v_fechaPago, ESTADOFACTURA = CASE WHEN RF.ESTADO = 'FINAL' THEN 'PAGADO' ELSE 'ERROR' END, MODIF_DATE = CURRENT_TIMESTAMP 
	, MODIF_USER = MODIF_USER|| '-IF002_PAGOS'
...
DELETE FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%SP_PAGO_FACTURAS%' AND CAST(DATETIME AS DATE) = CURRENT_DATE;
SELECT * FROM EXT.PAGO_FACTURAS_LOAD;
CALL EXT.SP_PAGO_FACTURAS('1689_IF002PAGOFAC_20252711CE01.csv');
SELECT * FROM EXT.CSE_DEBUG WHERE PROCESO LIKE '%SP_PAGO_FACTURAS%' AND CAST(DATETIME AS DATE) = CURRENT_DATE;
SELECT * FROM EXT.FACTURAS WHERE IDFACTURA = 816;
