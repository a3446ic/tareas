CREATE OR REPLACE FUNCTION EXT.GET_PAGO_FACTURAS_ERROR() 
/*
	----------------------------------------------------------------------------------------------- 
...
*/
	...
	BEGIN
		RETURN SELECT 
			...
			FROM EXT.PAGO_FACTURAS RF
			WHERE RF.BATCHNAME IN (SELECT BATCHNAME FROM EXT.REGISTRO_INTERFACES WHERE NOTIFICATION = 0 AND BATCHNAME LIKE '%PAGOFAC%') 
			AND ESTADO = 'E'
	...
	SELECT * FROM EXT.GET_PAGO_FACTURAS_ERROR();
	END;